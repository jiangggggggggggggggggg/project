...
MyDrawPanel::MyDrawPanel(wxWindow* parent)
    ...
    m_selectedIndex(-1), m_selectedWireIndex(-1),
    m_showGrid(true), m_showPins(true), m_gridSize(20),
    m_wireMode(WIRE_ORTHOGONAL), // 默认使用直角模式
...
bool MyDrawPanel::IsPointNearLine(const wxPoint& point, const Wire& wire, int tolerance) {
    if (m_wireMode == WIRE_STRAIGHT) {
        ...
        // 检测三段线
        Wire segment1 = { wire.start, mid1 };
        Wire segment2 = { mid1, mid2 };
        Wire segment3 = { mid2, wire.end };

        return IsPointNearLine(point, segment1, tolerance) ||
            IsPointNearLine(point, segment2, tolerance) ||
            IsPointNearLine(point, segment3, tolerance);
    }
}
...
void MyDrawPanel::OnPaint(wxPaintEvent&) {
    ...
    if (gc) {
        ...
        // 绘制正在画的线
        if (m_isDrawingWire) {
            double width = (m_connectedEndPin.gateIndex != -1) ? 3.0 : 2.0;
...
void MyDrawPanel::OnMouseMove(wxMouseEvent& evt) {
    ...
    if (m_isDrawingWire || (!m_isDraggingGate && !m_isDrawingWire)) {
        ...
        if (nearestPin.gateIndex != -1) {
            ...
            m_hoveredPin = { -1, -1, wxPoint() };
            if (!m_isDrawingWire && !m_isDraggingGate) {
